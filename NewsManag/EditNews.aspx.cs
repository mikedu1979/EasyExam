...
namespace EasyExam.NewsManag
{
	...
	public partial class EditNews : System.Web.UI.Page
	{
		...
		protected void ButInput_Click(object sender, System.EventArgs e)
		{
			...
			{
				this.RegisterStartupScript("newWindow","<script language='javascript'>alert('通知标题不能为空！')</script>");
				return;
			}
			if (txtNewsContent.Text.Trim()=="")
			{
				this.RegisterStartupScript("newWindow","<script language='javascript'>alert('通知内容不能为空！')</script>");
				return;
			}
			...
			if (strTmp.Trim()!="")
			{
				this.RegisterStartupScript("newWindow","<script language='javascript'>alert('此通知标题已经存在，请重新输入！')</script>");
				return;
			}
			...
			try
			{
				...
				this.RegisterStartupScript("newWindow","<script language='javascript'>alert('修改通知成功！');try{ window.opener.RefreshForm() }catch(e){};window.close();</script>");
			}
			catch
			{
				ObjTran.Rollback();
				this.RegisterStartupScript("newWindow","<script language='javascript'>alert('修改通知失败！')</script>");
			}
			finally
			{
				ObjConn.Close();
