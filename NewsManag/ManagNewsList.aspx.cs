		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				...
				else
				{
					...
					ButDelete.Attributes.Add("onclick","javascript:{if(confirm('确实要删除选择通知吗？')==false) return false;}");
		...
		private void ShowData(string strSql)
		{
			...
			{				
				...
				if ((myLoginID.Trim().ToUpper()=="ADMIN")||(myLoginID.Trim().ToUpper()==DataGridNews.Items[i].Cells[5].Text.Trim().ToUpper()))
				{
					...
					LBDel.Attributes.Add("onclick", "javascript:{if(confirm('确定要删除选择通知吗？')==false) return false;}");
				}
				else
				{
