	...
	public partial class ManagBookList : System.Web.UI.Page
	{
		...
		private void ShowData(string strSql)
		{
			...
			{				
				...
				{
					//LBEditBook.Attributes.Add("onclick", "javascript:jscomNewOpenByFixSize('EditBook.aspx?SectionID="+DataGridBook.Items[i].Cells[0].Text+"&SectionName="+DataGridBook.Items[i].Cells[5].Text+"','EditBook',1055,896); return false;");
					//LBDel.Attributes.Add("onclick", "javascript:{if(confirm('确定要删除选择章节吗？')==false) return false;}");
					LBEditBook.Attributes.Add("onclick", "javascript:jscomNewOpenByFixSize('uploadpdf.aspx?SectionID=" + DataGridBook.Items[i].Cells[0].Text + "&SectionName=" + DataGridBook.Items[i].Cells[5].Text + "','EditBook',1055,896); return false;");
					LBDel.Attributes.Add("onclick", "javascript:{if(confirm('确定要删除选择章节吗？')==false) return false;}");
				}
				else
				{
